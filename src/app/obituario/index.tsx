...
export default function Obituario() {
  ...
  const filteredPessoas = sepultados?.pessoas?.filter((pessoa: Pessoa) => {
    if (!searchTerm) return false // Não mostra nada se não houver busca
    
    switch (searchType) {
      case "nome":
        return pessoa.nome.toLowerCase().includes(searchTerm.toLowerCase())
      case "cpf":
        // Busca por CPF - remove formatação e compara apenas números
        ...
        return pessoaCPF.includes(searchCPF)
      case "data":
        // Busca por data em qualquer formato (nascimento, falecimento ou sepultamento)
        ...
        const falecimentoFormatada = pessoa.data_falecimento_formatada.replace(/\D/g, "")
        const sepultamentoFormatada = pessoa.data_sepultamento_formatada.replace(/\D/g, "")
        
        const nascimentoOriginal = pessoa.data_nascimento.replace(/\D/g, "")
        const falecimentoOriginal = pessoa.data_falecimento.replace(/\D/g, "")
        const sepultamentoOriginal = pessoa.data_sepultamento.replace(/\D/g, "")
        
        return nascimentoFormatada.includes(searchDate) || 
               falecimentoFormatada.includes(searchDate) || 
               sepultamentoFormatada.includes(searchDate) ||
               nascimentoOriginal.includes(searchDate) ||
               falecimentoOriginal.includes(searchDate) ||
               sepultamentoOriginal.includes(searchDate)
      default:
        return false
    }
  }) || []
